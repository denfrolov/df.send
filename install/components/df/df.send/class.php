	...
	public function sendMessageAction(): array
	{
		...
		if ($_REQUEST['IBLOCK_ID'] == $arParams['IBLOCK_ID']) {
			...
			if ($recaptchaPublicKey && $recaptchaSecretKey && $useRecaptcha == 'Y') {
				...
				file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/log.php', '<script>console.log(' . json_encode($recaptcha, JSON_PARTIAL_OUTPUT_ON_ERROR) . ')</script>', FILE_APPEND);
		...
		while ($arProperty = $rsProperty->Fetch()) {
			...
			if ($arProperty['USER_TYPE'] != 'HTML' && $arProperty['PROPERTY_TYPE'] != 'L') {
				...
				if ($arProperty['PROPERTY_TYPE'] == 'L') {
					...
					$property_enums = CIBlockPropertyEnum::GetList(array("SORT" => "ASC", "ID" => "ASC"), array("IBLOCK_ID" => $arParams['IBLOCK_ID'], "CODE" => $arProperty['CODE']));
					while ($enum_fields = $property_enums->GetNext()) {
